...
export default function ARScanScreen() {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    try {
      ...
      wsRef.current = ws;
      ws.onopen = () => {
        ws.send(JSON.stringify({ mode: selectedMode, level, content: '' }));
      };
      ws.onmessage = (event) => {
        try {
  ...
  }, [selectedMode, level, streamingText]);
  ...
  }, [connectWebSocket, step, level, selectedMode]);

  const recognizeAndTutor = useCallback(async () => {
    if (!cameraRef.current || analyzing || loading) return;
    setAnalyzing(true);
    try {
  ...
  }, [analyzing, loading, selectedMode, level]);
      ...
      if (uri) {
        ...
        try {
          ...
          if (text) {
            ...
            const reply = messages.find((m) => m.role === 'teacher')?.text || 'Good question. Try the next small step.';
            speak(reply);
          }
        } finally {
          setListening(false);
  ...
  }, [steps, messages, penNotes, router]);
  ...
  if (!permission.granted) {
    return (
      <View style={styles.permissionContainer}>
        ...
        <Text style={styles.permissionText}>Camera access needed for live AR tutoring</Text>
          ...
          <TouchableOpacity style={styles.startBtn} onPress={recognizeAndTutor} disabled={analyzing || loading}>
            {(analyzing || loading) ? (
              ...
              <>
                ...
                <Text style={styles.startBtnText}>Start Live AR Tutor</Text>
              </>
            )}
          </TouchableOpacity>
        )}
